-- local save = require "vn.save"

local M = {}

local app_id = "Untitled MatchaNovel project"
local path_settings = sys.get_save_file(app_id, "settings.dat")
local path_global = sys.get_save_file(app_id, "global.dat")
...
local function get_path(slot, quick, auto)
	if quick then 
		...
		return sys.get_save_file(app_id, "quicksave_"..n..".dat")
	elseif auto then 
		local n = get_number_string(slot, autosaves_limit)
		return sys.get_save_file(app_id, "autosave_"..n..".dat")
	else
		local n = get_number_string(slot, saves_limit)
		return sys.get_save_file(app_id, "save_"..n..".dat")
	end
end
...
function M.load(slot)
	loaded_table = sys.load(get_path(slot, false, false))
...
function M.quickload(number)
	...
	loaded_table = sys.load(get_path(slot, true, false))
	local loaded = is_table(loaded_table)
	if loaded then
		M.state.pos = loaded_table.pos
		M.state.log = loaded_table.log
...
function M.open_folder()
	sys.open_url(sys.get_save_file(app_id, ""))
end
...
local function load_global()
	local loaded_table = sys.load(path_global)
	if loaded_table then
		M.global = loaded_table
	end
end

local function write_global()
	flag_write_global = false
	sys.save(path_global, M.global)
end
