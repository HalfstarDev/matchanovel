...
local system = require "matchanovel.system"
...
local function scene(args)
	...
	M.post("background", "scene", {scene = scene})
	matchascript.next()
end
...
local function show(args)
	...
	M.post("sprites", "show", {name = name, at = at})
	matchascript.next()
end

local function hide(args)
	M.post("sprites", "hide", {name = args[1]})
	matchascript.next()
end
...
local function play(args)
	...
	if args[2] then
		...
		file = args[1]
		group, type = save.get_var("sound.group")
		if group == nil then
			group = "music"
		end
	end
	...
	elseif group == "sound" then 
		M.post("sound", "play_sound", {id = file})
...
script_definition.functions["return"] = action_return
script_definition.functions["if"] = action_if
